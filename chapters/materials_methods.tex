...
Three growing media treatments were established for each mint species: an unamended peat control, and peat amended with either 15~g or 30~g of biochar per pot. Each treatment was replicated in three pots, giving nine pots per species and 27 pots in total across the three species (Table~\ref{tab:media}). The biochar was incorporated in a dry state, without prior soaking or pre-wetting.

Planting was staggered. \textit{Mentha piperita} and \textit{Mentha crispata} were potted on 15 September 2026, and \textit{Mentha spicata} was potted subsequently.

\begin{table}[htbp]
  \centering
  \caption[Growing media treatments]{Growing media treatments applied to each mint species.}
  \label{tab:media}
  \begin{tabular}{lrr}
    \toprule
    Treatment & Biochar per pot (g) & Replicate pots \\
    \midrule
    Control (unamended peat) & 0  & 3 \\
    Low biochar rate         & 15 & 3 \\
    High biochar rate        & 30 & 3 \\
    \midrule
    Total per species        &    & 9 \\
    \bottomrule
  \end{tabular}
...
\textcolor{red}{TODO: Still required for this section -- (i) the pot volume and the mass or volume of peat per pot, without which the 15~g and 30~g rates cannot be expressed as a proportion of the substrate; (ii) the biochar feedstock and pyrolysis temperature; (iii) whether the biochar was mixed through the peat or layered; and (iv) the planting date for \textit{M. spicata}, together with the resulting difference in plant age at sampling.}
